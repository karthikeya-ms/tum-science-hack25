...
def verify_sectors():
    ...
    try:
        ...
        for i, sector in enumerate(sectors, 1):
            ...
            print(f"  Assigned NGO: {sector.assigned_ngo.userName if sector.assigned_ngo else 'None'}")
            print(f"  Assigned Team Lead: {sector.assigned_team_lead.userName if sector.assigned_team_lead else 'None'}")
            print(f"  Assigned Operator: {sector.assigned_operator.userName if sector.assigned_operator else 'None'}")
            
            # Get geometry info (first few coordinates)
            geom_query = session.execute(
                text("SELECT ST_AsText(geometry) as wkt FROM sectors WHERE id = :sector_id"),
                {"sector_id": sector.id}
            ).fetchone()
        ...
        avg_risk = sum(sector.risk_probability for sector in sectors) / len(sectors) if sectors else 0
        
        print(f"Total Area: {total_area:.2f} square meters ({total_area/1000000:.2f} square kilometers)")
