...
class User(Base):
    ...
    assigned_team_lead_sectors = relationship('Sector', foreign_keys='Sector.assigned_to_team_lead_id', back_populates='assigned_team_lead')
    assigned_operator_sectors = relationship('Sector', foreign_keys='Sector.assigned_to_operator_id', back_populates='assigned_operator')
...
class Sector(Base):
    ...
    assigned_ngo = relationship(
        'User',
        foreign_keys=[assigned_to_ngo_id],
        back_populates='assigned_ngo_sectors'
    )
    ...
    def __repr__(self):
        ngo_name = self.assigned_ngo.userName if self.assigned_ngo else "None"  # Changed from username to userName
        team_lead_name = self.assigned_team_lead.userName if self.assigned_team_lead else "None"  # Changed from username to userName
        operator_name = self.assigned_operator.userName if self.assigned_operator else "None"  # Changed from username to userName
        return (
            f"<Sector(id={self.id}, "
